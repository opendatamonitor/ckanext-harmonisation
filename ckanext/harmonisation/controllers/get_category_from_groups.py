...
def Get_category_from_groups():
  ...
  odm_harmonised_collection=db.odm_harmonised_cat
  datasets=list(odm_harmonised_collection.find());
  i=0
  while i<len(datasets):
	if 'groups' in datasets[i].keys() and len(datasets[i]['groups'])>0 and 'category' not in datasets[i]['extras'].keys() and datasets[i]['platform']=='ckan':
