...
def HarmoniseTags(cat_url):
  ...
  while i<len(datasets):
	  ...
	  if 'harmonised_tags' not in datasets[i]['harmonised'].keys():
		  try:
			...
			datasets[i]['harmonised'].update({'harmonised_tags_date':datetime.now()})
			collection.save(datasets[i])
  ...
  text_file.write(str(datetime.now())+' - harmonisation_engine - '+"Tags harmonised in: "+str(counter)+ " datasets"+'\n')
...
def HarmoniseExtras(cat_url):
  ...
  text_file.write(str(datetime.now())+' - harmonisation_engine - '+"Extras harmonised in: "+str(counter)+ " datasets"+'\n')
...
def HarmoniseStringsToIntegers(cat_url):
  ...
  while i<len(datasets):
	if 'harmonised' not in datasets[i].keys():
	  datasets[i].update({'harmonised':{}})
	  collection.save(datasets[i])
	if 'harmonised_StringsToIntegers' not in datasets[i]['harmonised'].keys():
		try:
		  ...
		  while j<len(resources):
			  ...
			  if 'size' in str(datasets[i]['resources'][j]) and datasets[i]['resources'][j]['size']!=None and datasets[i]['resources'][j]['size']!="" and datasets[i]['resources'][j]['size']!="None":
				size=datasets[i]['resources'][j]['size']
				try:

				  datasets[i]['resources'][j].update({"size":int(size)})
		  ...
		  j=0
		  datasets[i]['harmonised'].update({'harmonised_StringsToIntegers':True})
		  datasets[i]['harmonised'].update({'harmonised_StringsToIntegers_date':datetime.now()})
		  collection.save(datasets[i])
		  i+=1
		except KeyError:
		  i+=1
  ...
  text_file.write(str(datetime.now())+' - harmonisation_engine - '+" Harmonised string values to int values in: "+str(counter)+ " fields"+'\n')
  message="Harmonised string values to int values in: "+str(counter)+ " fields"+'\n'
  return message
...
def HarmoniseDatesLabels(cat_url):
  ...
  if dates_dictionary2==None:dates_dictionary2={}
  dates_dictionary3=dates_dict_catalogue.find_one({'cat_url':str(cat_url)})
  dates_dictionary={}
  i=0
  while i<len(dates_dictionary1):
	dates_dictionary.update({dates_dictionary1.keys()[i]:dates_dictionary1[dates_dictionary1.keys()[i]]})
	i+=1
  i=0
  while i<len(dates_dictionary2):
	dates_dictionary.update({dates_dictionary2.keys()[i]:dates_dictionary2[dates_dictionary2.keys()[i]]})
	i+=1
  i=0
  while i<len(dates_dictionary3):
	dates_dictionary.update({dates_dictionary3.keys()[i]:dates_dictionary3[dates_dictionary3.keys()[i]]})
	i+=1
  count=0
			  ...
			  while j<len(extras_keys):
				...
				while k<len(datemappings_keys):
				  ...
				  if extras_keys[j]==datemappings_keys[k]:
					  extras[str(datemappings[str(datemappings_keys[k])])] = extras[str(extras_keys[j])]
					  del extras[str(extras_keys[j])]
					  collection.save(datasets[i])
					  counter+=1
					  datasets[i]['harmonised'].update({'harmonised_DatesLabels':True})
					  datasets[i]['harmonised'].update({'harmonised_DatesLabels_date':datetime.now()})
					  collection.save(datasets[i])
				  k+=1
  ...
  text_file.write(str(datetime.now())+' - harmonisation_engine - '+" Harmonised date labels in: "+str(counter)+ " datasets."+'\n')
  message="Harmonised date labels in: "+str(counter)+ " datasets."+'\n'
  return message
...
def HarmoniseReleaseDates(cat_url):
		...
		try:
		  ...
		  if extras!="":
			  ...
			  try:
				...
				if release_date1=="" or release_date1==None:

				  bad_date=re.findall(r'\d+',str(release_date.encode('utf-8')))
				  p=0
				  ...
				  while p<len(bad_date):
					...
					if len(bad_date1)!=4:
					  ...
					  datasets[i]['extras'].update({"date_released":bad_release_date})
				  ...
				  counter+=1
				  datasets[i]['extras'].update({"date_released":release_date1})
				  datasets[i]['harmonised'].update({'harmonised_ReleaseDates':True})
				  datasets[i]['harmonised'].update({'harmonised_ReleaseDates_date':datetime.now()})
				  collection.save(datasets[i])
  ...
  text_file.write(str(datetime.now())+' - harmonisation_engine - '+" Harmonised date_released in: "+str(counter)+ " datasets"+'\n')
  message="Harmonised date_released in: "+str(counter)+ " datasets"+'\n'
  return message
...
def HarmoniseUpdateDates(cat_url):
		  ...
		  if extras!="":
			  ...
			  try:
				...
				update_date1=parse(update_date)
				  ...
				  #if date parsing goes wrong
				if update_date1=="" or update_date1==None:

				  bad_date=re.findall(r'\d+',str(update_date.encode('utf-8')))
				  p=0
				  ...
				  while p<len(bad_date):
					...
					if len(bad_date1)!=4:
					  ...
					  datasets[i]['extras'].update({"date_updated":bad_update_date})
					  datasets[i]['harmonised'].update({'harmonised_UpdateDates':True})
					  datasets[i]['harmonised'].update({'harmonised_UpdateDates_date':datetime.now()})
					  collection.save(datasets[i])
					  counter+=1
					  p=len(bad_date)
				  ...
				  counter+=1
				  datasets[i]['extras'].update({"date_updated":update_date1})
				  datasets[i]['harmonised'].update({'harmonised_UpdateDates':True})
				  datasets[i]['harmonised'].update({'harmonised_UpdateDates_date':datetime.now()})
				  collection.save(datasets[i])
  ...
  text_file.write(str(datetime.now())+' - harmonisation_engine - '+" Harmonised date updated in: "+str(counter)+ " datasets"+'\n')
  message="Harmonised date updated in: "+str(counter)+ " datasets"+'\n'
  return message
...
def HarmoniseMetadataCreated(cat_url):
  ...
  while i<len(datasets):
	  if 'harmonised' not in datasets[i].keys():
		datasets[i].update({'harmonised':{}})
		collection.save(datasets[i])
	  if "metadata_created" in datasets[i].keys() and 'harmonised_MetadataCreated' not in datasets[i]['harmonised'].keys():
		  ...
		  try:
			...
			datasets[i]['harmonised'].update({'harmonised_MetadataCreated':True})
			datasets[i]['harmonised'].update({'harmonised_MetadataCreated_date':datetime.now()})
			collection.save(datasets[i])
			counter+=1
		  except:
			try:
...
def HarmoniseMetadataModified(cat_url):
  ...
  while i<len(datasets):
	  if 'harmonised' not in datasets[i].keys():
		datasets[i].update({'harmonised':{}})
		collection.save(datasets[i])
	  if "metadata_modified" in datasets[i].keys() and 'harmonised_MetadataModified' not in datasets[i]['harmonised'].keys():
...
def HarmoniseFormats(cat_url):
 ...
 #from dictionaries import basic_formats_dict
  ...
  if formats_dictionary2==None:formats_dictionary2={}
  formats_dictionary3=formats_dict_catalogue.find_one({'cat_url':str(cat_url)})
  ...
  while i<len(formats_dictionary1):
	formats_dictionary.update({formats_dictionary1.keys()[i]:formats_dictionary1[formats_dictionary1.keys()[i]]})
	i+=1
  i=0
  while i<len(formats_dictionary2):
	formats_dictionary.update({formats_dictionary2.keys()[i]:formats_dictionary2[formats_dictionary2.keys()[i]]})
	i+=1
  i=0
  ...
  text_file.write(str(datetime.now())+' - harmonisation_engine - '+" Harmonised formats in: "+str(counter)+ " datasets"+'\n')
  i1=0
  text_file.write('Unharmonised Formats Found are: '+'\n')
  while i1<len(unharmonised_formats):
	text_file.write("'"+str(unharmonised_formats[i1])+"':"+"'',"+'\n')
	i1+=1
  print("The unharmonised formats are in the harmonisation_engine.log file."+'\n'+'\n'+'\n')
...
def HarmoniseBadFormats(cat_url):
		...
		try:
		  ...
		  while j<len(resources):
			...
			if (str(format1.encode('utf-8')) not in keys or str(format1.encode('utf-8')).lower() not in keys) and (', ' in str(format1.encode('utf-8')) or '(' in str(format1.encode('utf-8'))) or format1=="" :
				...
				if format_find!='' and format_find!=None and format_find!="html":
				  try:
					  datasets[i]['resources'][j].update({"format":str(format_find).lower()})
  ...
  text_file.write(str(datetime.now())+' - harmonisation_engine - '+" Harmonised bad formats in: "+str(counter)+ " resources"+'\n')
  message="Harmonised bad formats in: "+str(counter)+ " resources"+'\n'
  return message
...
def HarmoniseSizes(cat_url):
  ...
  text_file.write(str(datetime.now())+' - harmonisation_engine - '+" Harmonised size in: "+str(counter)+ " resources."+'\n')
...
def HarmoniseMimetypes(cat_url):
	...
	if 'harmonised_Mimetypes' not in datasets[i]['harmonised'].keys():
		try:
		  ...
		  while j<len(resources):
			  ...
			  if 'mimetype' not in str(datasets[i]['resources'][j]) or datasets[i]['resources'][j]['mimetype']==None:
				...
				try:
				  ...
				  datasets[i]['harmonised'].update({'harmonised_Mimetypes_date':datetime.now()})
				  collection.save(datasets[i])
  ...
  print("Broken links found : "+str(counter_broken_links))
  text_file.write(str(datetime.now())+' - harmonisation_engine - '+" Harmonised resources in: "+str(counter)+ " resources."+'\n')
...
def HarmoniseNumTagsAndResources(cat_url):
  ...
  while i<len(datasets):
	...
	if 'harmonised' not in datasets[i].keys():
	  datasets[i].update({'harmonised':{}})
	  collection.save(datasets[i])
	if 'harmonised_NumTagsAndResources' not in datasets[i]['harmonised'].keys():
		try:
		  resources=datasets[i]['resources']
		  ...
		  if 'num_tags' not in str(datasets[i]) :
			...
			datasets[i]['harmonised'].update({'harmonised_NumTagsAndResources':True})
			datasets[i]['harmonised'].update({'harmonised_NumTagsAndResources_date':datetime.now()})
			collection.save(datasets[i])
			counter+=1
  ...
  datasets[:]=[]
  print("harmonised num tags and num resources in "+str(counter)+" datasets")
  text_file.write(str(datetime.now())+' - harmonisation_engine - '+" Harmonised num tags and num resources in: "+str(counter)+ " datasets"+'\n')
  message="Harmonised num tags and num resources in: "+str(counter)+ " datasets"+'\n'
  return message
...
def HarmoniseCatalogueUrl():
  ...
  while i<len(jobs):
	...
	if 'http://' in jobs[i]['cat_url']:
	  ...
	  jobs1.update({str(url_main1):str(jobs[i]['cat_url'].encode('utf-8'))})
  ...
  while endpoint<MaxDocsInDB:
	...
	while i<len(datasets):
		  ...
		  try:
			...
			if url3!="" and url3!=None:
			  if 'https' in url3:
				url3=url3[8:]
			  else:url3=url3[7:]
			  if '/' in url3:
				url3=url3[0:url3.find('/')].replace('_','.')
			  else:url3=url3.replace('_','.')
			  if 'www.' in url3:
				url3=url3[4:]
		  ...
		  try:
			...
			if url1!="" and url1!=None:
			  if 'https' in url1:
				url1=url1[8:]
			  else:url1=url1[7:]
			  if '/' in url1:
				url1=url1[0:url1.find('/')].replace('_','.')
			  else:url1=url1.replace('_','.')
			  if 'www.' in url1:
				url1=url1[4:]
		  ...
		  if url1!="" and url1!=None and found==False:

			k=0
			while k<len(keys):
			  if str(keys[k].encode('utf-8')) == str(url1.encode('utf-8')):
		  ...
		  if url2!="" and url2!=None and found==False:

			k=0
			while k<len(keys):
			  if str(keys[k].encode('utf-8')) == str(url2.encode('utf-8')):
		  ...
		  if url3!="" and url3!=None and found==False:
			k=0

			while k<len(keys):
			  if str(keys[k].encode('utf-8')) == str(url3.encode('utf-8')):
	...
	while i<len(datasets):
	  ...
	  try:
		...
		#	url1=url1[4:]
		  ...
		  while k<len(keys):
			if  str(url1) in str(keys[k]) or str(url1) == str(keys[k]) or  str(keys[k]) in str(url1) :

			  datasets[i].update({"platform":str(jobs1[str(keys[k])])})
			  collection.save(datasets[i])
			  counter+=1
			else :
			 pass
...
def HarmoniseLicenses(cat_url):
  ...
  text_file.write(str(datetime.now())+' - harmonisation_engine - '+" Harmonised license in: "+str(counter)+ " datasets"+'\n')
  i1=0
  text_file.write(str(datetime.now())+'The Unharmonised Licenses are: '+'\n')
  while i1<len(unharmonised_licenses):
	text_file.write("'"+str(unharmonised_licenses[i1])+"':"+"'',"+'\n')
	i1+=1
...
## function that harmonises the language labels found in extras according to the existing dictionary in dictinaries.py file
def HarmoniseLanguageLabels(cat_url):
			  ...
			  while j<len(extras_keys):
				...
				while k<len(languagemappings_keys):
				  ...
				  if extras_keys[j]==languagemappings_keys[k]:
					  ...
					  extras[str(languagemappings[str(languagemappings_keys[k])])] = extras[str(extras_keys[j])]
					  del extras[str(extras_keys[j])]
					  datasets[i]['harmonised'].update({'harmonised_LanguageLabels':True})
					  datasets[i]['harmonised'].update({'harmonised_LanguageLabels_date':datetime.now()})
					  collection.save(datasets[i])
					  counter+=1
				  k+=1
  ...
  text_file.write(str(datetime.now())+' - harmonisation_engine - '+" Harmonised language labels in: "+str(counter)+ " datasets."+'\n')
  message="Harmonised language labels in: "+str(counter)+ " datasets."+'\n'
  return message
...
def HarmoniseLanguages(cat_url):
		...
		try:
		  ...
		  if extras!="":
			  ...
			  while j<len(languagemappings_keys):
				  if str(language.encode('utf-8')) == languagemappings_keys[j] or str(language.encode('utf-8')).lower() == languagemappings_keys[j].lower():
					datasets[i]['extras'].update({"language":str(languagemappings[str(languagemappings_keys[j])])})
					datasets[i]['harmonised'].update({'harmonised_Languages':True})
					datasets[i]['harmonised'].update({'harmonised_Languages_date':datetime.now()})
					collection.save(datasets[i])
					counter+=1
				  j+=1
  ...
  text_file.write(str(datetime.now())+' - harmonisation_engine - '+" Harmonised language in: "+str(counter)+ " datasets."+'\n')
...
def HarmoniseCategoryLabels(cat_url):
  ...
  if categories_dictionary2==None:categories_dictionary2={}
  categories_dictionary3=categories_dict_catalogue.find_one({'cat_url':str(cat_url)})
  categories_dictionary={}
  i=0
  while i<len(categories_dictionary1):
	categories_dictionary.update({categories_dictionary1.keys()[i]:categories_dictionary1[categories_dictionary1.keys()[i]]})
	i+=1
  i=0
  while i<len(categories_dictionary2):
	categories_dictionary.update({categories_dictionary2.keys()[i]:categories_dictionary2[categories_dictionary2.keys()[i]]})
	i+=1
  i=0
  ...
  text_file.write(str(datetime.now())+' - harmonisation_engine - '+" Harmonised category labels in: "+str(counter)+ " datasets."+'\n')
  message="Harmonised category labels in: "+str(counter)+ " datasets."+'\n'
  return message
...
def HarmoniseCategories(cat_url):
		  ...
		  try:
			...
			if language_detection!='en':
				...
				while k<len(dictionary):
				  if "id" in str(dictionary[k].keys()[0].encode('utf-8')):
					temp_word=dictionary[k].keys()[1].encode('utf-8')
				  else:temp_word=dictionary[k].keys()[0].encode('utf-8')
				...
				if category  in dictionary1:
				  while j<len(dictionary):
					...
					if category in dictionary[j]:
					  ...
					  translation=dictionary[j][category.encode('utf-8')]
					  print(category.encode('utf-8')+' --> '+translation.encode('utf-8'))
					  datasets[i]['extras'].update({"category":translation.encode('utf-8')})
					  datasets[i]['harmonised'].update({'harmonised_Categories':True})
					  datasets[i]['harmonised'].update({'harmonised_Categories_date':datetime.now()})
					  found=True
					  try:
						collection.save(datasets[i])
						counter+=1
				...
				if found==False and category.encode('utf-8') not in dictionary1 :
				  ...
				  if category_en.encode('utf-8')!="":
					try:
					  collection_dict.save({category.encode('utf-8'):category_en.encode('utf-8')})
					  #counter+=1
					except:
					  print('storage error')
					datasets[i]['extras'].update({"category":str(category_en.encode('utf-8'))})
					datasets[i]['harmonised'].update({'harmonised_Categories':True})
					datasets[i]['harmonised'].update({'harmonised_Categories_date':datetime.now()})
					print(category.encode('utf-8')+' -//-> '+category_en.encode('utf-8'))
					try:
					  collection.save(datasets[i])
					  counter+=1
					except:
  ...
  text_file.write(str(datetime.now())+' - harmonisation_engine - '+" Harmonised category value in: "+str(counter)+ " datasets."+'\n')
  message="Harmonised category value in: "+str(counter)+ " datasets."+'\n'
  return message
...
def country_names(dataset={}):
        ...
        if not country_name:
            ...
            if ('author_email' in dataset.keys() and email_validator(dataset['author_email'])):
        ...
        if not country_name:
            ...
            if ('maintainer_email' in dataset.keys() and email_validator(dataset['maintainer_email'])):
        ...
        if not country_name:
            ...
            if ('contact-email' in dataset['extras'].keys() and email_validator(dataset['extras']['contact-email'])):
                ...
                if result:
                    ...
                    return country_name
        if not country_name:
            if 'eu_country' in dataset['extras'].keys() and dataset['extras']['eu_country']:
                match=dataset['extras']['eu_country']
                result=countries_table.find_one({'$or':[{'fips':match.upper()},{'ISO':match.upper()}]})
                if result:
                    country_name=result['ISO']
                    return country_name
                    # continue
        if not country_name:
            if 'harvest_catalogue_url' in dataset['extras'].keys() and dataset['extras']['harvest_catalogue_url']:
            ...
            if 'url' in dataset.keys() and dataset['url']:
                ...
                for tt in tokens:
                    ...
                    if result:
                        ...
                        return country_name
        if not country_name:
            if 'country' in dataset['extras'].keys() and dataset['extras']['country']:
                ...
                if counter==1:
                    ...
                    for tt in tokens:
                        if result['name']==tt:
                            ...
                            result=countries_table.find_one({'$or':[{'fips':country_name},{'ISO':country_name}]})
                            if result:
                                country_name=result['ISO']
                                return country_name
                            else:
                                print('Could not find country for %s, for dataset with _id: ObjectId(\'%s\')' % (country_name,dataset['_id']))
                                country_name=''
                            break
                elif counter>1:
                    try:
                        ...
                        for result in results:
                            for tt in tokens:
                                if result['name']==tt:
                                    found=countries_table.find_one({'$or':[{'fips':result['country code']},{'ISO':result['country code']}]})
                                    if found:
                        ...
                        if len(list_country_names)==1:
                            ...
                            print('Different country names found %s for tokens: %s, for dataset with _id: ObjectId(\'%s\')' % (', '.join(str(x) for x in list_country_names), ', '.join(str(x) for x in related_tokens), dataset['_id']))
        ...
        if not country_name:
            if 'bbox-west-long' in dataset['extras'].keys() and \
                ...
                bbox=BoundingBox([(float(dataset['extras']['bbox-west-long']),float(dataset['extras']['bbox-south-lat'])),(float(dataset['extras']['bbox-east-long']),float(dataset['extras']['bbox-north-lat']))])
                list_country_names=[c.name for c in country_subunits_containing_point(lon=bbox.center[0],lat=bbox.center[1])]
                if len(list_country_names)==1:
                    country_name=list_country_names[0]
                    return country_name
                else:
                    print(list_country_names)
                    results=cities_table.find({'name':{'$in':list_country_names}})
                    list_country_names=[]
                    related_tokens=[]
                    for result in results:
                        found=countries_table.find_one({'$or':[{'fips':result['country code']},{'ISO':result['country code']}]})
        ...
        if not country_name:
            print ('Still cannot find country name for dataset with _id: ObjectId(\'%s\') ' % dataset['_id'] )
            # continue
		    ...
		    if 'harmonised_Countries' not in dataset['harmonised'].keys():
			    total_counter=total_counter+1
		            ...
		            if country_name:
		                ...
		                db.update({'_id':dataset['_id']},{'$set':{'country':country_name}})
		                dataset['harmonised'].update({'harmonised_Categories':True})
		                dataset['harmonised'].update({'harmonised_Categories_date':datetime.now()})
		                db.save(dataset)
		            else:
                ...
                if no_countriesNotFound > 0:
                    print ('%i country names where not possible to be found\n' % no_countriesNotFound)
                ...
                if no_datasetWithoutCategory>0:
                    print('%i datasets has no category field\n' % no_datasetWithoutCategory)
                if (total_counter % 1000) > 0:
...
def HarmoniseCategoryValues(cat_url,logger,dataset={}):
	...
	try:
        ...
        while count<len(category_values):
          ...
          if '(dot)' in key:
            key1=key.replace('(dot)','.')
            category_values.update({key1:category_values[str(category_values.keys()[count])]})
...
def FinishHarmonisationEngine():
  ...
  text_file.write(str(datetime.now())+' - harmonisation_engine  - '+" Harmonation engine Finished.")
...
def IdentifyLanguageFromNotes():
  ...
  while endpoint<MaxDocsInDB:
	...
	i=0
	while i<len(datasets):
		if 'Harmonization_finished' not in datasets[i].keys() and 'language' not in str(datasets[i]['extras']) :
			...
			if language_detection!='':
				...
				datasets[i]['extras'].update({'language':str(language_detection)})
				collection.save(datasets[i])
				counter+=1
			i+=1
...
def mails():
  ...
  text_file_mails = open('/var/local/ckan/default/pyenv/src/harmonisation_engine/emails.txt', "a")
  while endpoint<MaxDocsInDB:
	endpoint=point+step
