...
def main(db,collection):
    ...
    while(more_datasets):
        try:
            ...
            datasets=client.find(
                # {'$match':{'pagerank':{'$exists':False}}},
            ...
            for dataset in datasets:
                ...
                try:
                    for p in providers:
                        pagerank[p.__class__.__name__]=p.get_rank(dataset['cat_url'])

                    client.update({'_id':dataset['_id']},{'$set':{'pagerank':pagerank}},multi= True)

                    if (total_counter % 10) == 0:
                        print ('%i catalogues ranked...' % total_counter)
                except KeyError as e:
                    print('KeyError: %s' % e)
                except urllib2.HTTPError,e:
                    print('url: %s, %s' % (dataset['cat_url'],e))
